...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentUser = {
        isLoggedIn: false,// scripts.js - Updated with sign-in functionality
document.addEventListener('DOMContentLoaded', function() {
    // Initialize user state
    let currentUser = {
        isLoggedIn: false,
    ...
    if (document.querySelector('.signin-form')) {
        ...
        signinForm.addEventListener('submit', function(e) {
            ...
            const userType = document.querySelector('.user-type.selected').classList.contains('buyer') ? 'Buyer' : 'Seller';
            
            // Simulated sign-in
            currentUser.isLoggedIn = true;
            alert(`Successfully signed in as ${userType}! Redirecting to marketplace...`);
            window.location.href = 'index.html';
        });
        
        // Social sign-in
        document.querySelectorAll('.signin-option').forEach(option => {
            option.addEventListener('click', function(e) {
                e.preventDefault();
                const provider = this.id.replace('-signin', '');
                const userType = document.querySelector('.user-type.selected').classList.contains('buyer') ? 'Buyer' : 'Seller';
                signUpWithProvider(provider.charAt(0).toUpperCase() + provider.slice(1));
                window.location.href = 'index.html';
            });
        });
    }
